...
class Star_skill_seed(Ut):
	...
	def test_case11(self):
		u'''判断首屏技能是否正确'''
		skill_num =self.data.get('skill_position')
		dev_skill_first_replace  = self.data.get('map')
		if not self.assertEqual(dev_skill_first_replace[skill_num],self.reward_pic):
	...
	def test_case12(self):
		...
		skill_num =self.data.get('skill_position')
		#获取每屏该坐标对应的图标
		dev_skill_all_replace  = [x[skill_num] for x in self.tc.all_banmian(self.data)]
		if not self.assertEqual(dev_skill_all_replace.count(self.reward_pic),len(dev_skill_all_replace)):
	...
	def test_case14(self):
		...
		if self.first_win_flag:
			...
			if not self.assertEqual(dev_replace,self_replace):
				self.fail.append(self.test_case14.__doc__)
		else:
			self.test_case04()
...
if __name__ =='__main__':
	...
	for i in range(30, 70, 2):
		...
		data = get_seed_data(2, i)
		if len(data) == 0:
			write_log('skill_null', 'id:{}\n'.format(i))
			continue
		try:
			tc = Star_skill_seed(data)
			result = tc.run_all_case(i)
			write_log('skill_detail', 'id:{},detail:{}\n'.format(i, result))
			if len(result) > 0:
				write_log('skill_fail', 'id:{},detail:{}\n'.format(i, result))
		except:
			write_log('skill_except', 'id:{}\n,detail:{}\n'.format(i, traceback.format_exc()))
	end = time.time() - start
	print(end)