...
class Star_reward_seed(Ut):
	...
	def __init__(self, data):
		...
		self.reward_coordinate = [0,3,12,15]
		self.reward_pic = "e"
		self.fail = []
	@property
	...
	#此处因为本身就是免费游戏,所以是否免费游戏标识应始终为0
	@property
	def test_case10(self):
		u'''判断是否免费游戏'''
		if self.first_win_flag:
			dev_free_flag = self.data.get('is_free')
			if not self.assertEqual(dev_free_flag,None):
	...
	def test_case11(self):
		u'''判断首屏奖励游戏坐标是否正确'''
		reward_num = self.reward_coordinate
		first_map = self.data.get('map')
		if not self.assertTrue(self.tc.cmp_reward_page(first_map,reward_num,self.reward_pic)):
	...
	def test_case12(self):
		...
		reward_num = self.reward_coordinate
		#获取所有版面信息
		all_map = self.tc.all_banmian(self.data)
		#获取每个版面奖励图标是否正确
		dev_skill_all_replace = [self.tc.cmp_reward_page(x,reward_num,self.reward_pic) for x in all_map]
		#判断是否所有的版面奖励图标都是正确
		if not self.assertEqual(dev_skill_all_replace.count(True), len(dev_skill_all_replace)):
	...
	def test_case13(self):
		...
		if self.first_win_flag:
			...
			if not self.assertEqual(dev_replace,self_replace):
				self.fail.append(self.test_case13.__doc__)
		else:
			self.test_case04
...
if __name__ == '__main__':
	...
	for i in range(2, 200000, 2):
		...
		data = get_seed_data(3, i)
		if len(data) == 0:
			write_log('reward_null', 'id:{}'.format(i))
			continue
		try:
			tc = Star_reward_seed(data)
			result = tc.run_all_case(i)
			write_log('reward_detail', 'id:{},detail:{}\n'.format(i, result))
			if len(result) > 0:
				write_log('reward_fail', 'id:{},detail:{}\n'.format(i, result))
		except:
			write_log('reward_except', 'id:{}\n,detail:{}\n'.format(i, traceback.format_exc()))
